...
namespace GitOut.Features.Git.Log
{
    public class LogEntriesViewModel : INotifyPropertyChanged
    {
        ...
        private readonly ILazyAsyncEnumerable<
        ...
        )
        {
            ...
            diffFiles = new SortedLazyAsyncCollection<
                IGitFileEntryViewModel,
                RelativeDirectoryPath
            >(
                relativePath =>
            ...
            Branches = root
                .Branches.Select(branch => new BranchNameViewModel(
                    branch,
                    repository,
                    notifier,
                    snack
        ...
        public ILazyAsyncEnumerable<IGitFileEntryViewModel, RelativeDirectoryPath> AllFiles =>
            allFiles;
        ...
        private async IAsyncEnumerable<IGitFileEntryViewModel> ListAllFilesAsync()
        {
            ...
            IDictionary<string, DirectoryScaffold> tree =
                new Dictionary<string, DirectoryScaffold>();
            int max = 0;
            await foreach (
                ...
                if (!tree.TryGetValue(item.Directory.Directory, out DirectoryScaffold? directory))
                {
                    ...
                    while (!current.IsRoot)
                    {
                        ...
                        if (
                            tree.TryGetValue(
                                parent.Directory,
                                out DirectoryScaffold? parentDirectory
                            )
                        )
                        {
        ...
        private void SelectItem(IGitFileEntryViewModel? entry)
        {
            if (
                entry is null
                || currentSource.Source is not IEnumerable<IGitFileEntryViewModel> items
            )
            {
                return;
            }
