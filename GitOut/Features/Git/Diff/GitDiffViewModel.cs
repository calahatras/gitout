    ...
    public class GitDiffViewModel : IDocumentSelectionViewModel
    {
        ...
        )
        {
            ...
            foreach (GitDiffHunk hunk in result)
            {
                ...
                Paragraph header = CreateHeaderParagraph(
                    hunk.Header.StrippedLine,
                    display.HeaderForeground
                );
                ...
                IEnumerable<Paragraph> highlighted = highlighter.Highlight(
                    lines.Select(line => line.StrippedLine),
                    new DiffLineHighlighter(lines)
                );
            ...
            static Paragraph CreateHeaderParagraph(string text, Brush foreground) =>
                new(new Run(text))
                {
        ...
        {
            private static readonly Brush RemovedLineBackground = new SolidColorBrush(
                Color.FromArgb(128, 200, 90, 90)
            );
            private static readonly Brush AddedLineBackground = new SolidColorBrush(
                Color.FromArgb(90, 90, 200, 90)
            );

            private readonly IReadOnlyList<HunkLine> lines;

            public DiffLineHighlighter(IEnumerable<HunkLine> lines) =>
                this.lines = lines.ToList().AsReadOnly();

            public void Decorate(TextElement paragraph, int lineNumber) =>
                paragraph.Background = lines[lineNumber].Type switch
                {
