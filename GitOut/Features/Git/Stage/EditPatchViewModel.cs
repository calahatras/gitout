...
public class EditPatchViewModel : INotifyPropertyChanged, IHunkLineVisitorProvider
{
    ...
    )
    {
        ...
        text = string.Join(
            Environment.NewLine,
            lines.Where(line => line.Type != DiffLineType.Removed).Select(line => line.StrippedLine)
        );
    ...
    public static EditPatchViewModel StageFrom(IHunkLineVisitor visitor)
    {
        ...
        int fromRangeIndex = preline.Type switch
        {
            DiffLineType.Header => preline.FromIndex!.Value,
            DiffLineType.Removed or DiffLineType.None => preline.FromIndex!.Value,
            _ => throw new InvalidOperationException("Preline is not of expected type"),
        };
        ...
        return postline is null
            ? throw new InvalidOperationException("Need a postline to create patch")
            : new EditPatchViewModel(fromRangeIndex, lines, preline, postline);
    }
    ...
    private class EditPatchHunkLineVisitor : IHunkLineVisitor
    {
        ...
        public IEnumerable<HunkLine> TraverseSelectionHunks() =>
            parent
                ...
                .Concat(
                    parent
                        .lines.Where(line => line.Type is DiffLineType.None or DiffLineType.Removed)
                        .Select(
                            (line, index) =>
                                HunkLine.AsRemoved(
                                    $"-{line.StrippedLine}",
