...
public class MouseWheelGestureTypeConverter : TypeConverter
{
    ...
    )
    {
        if (value is string str)
        {
            ...
            return Enum.IsDefined(typeof(MouseWheelAction), parts[^1])
                    ? new MouseWheelGesture(Enum.Parse<MouseWheelAction>(parts[^1]), modifier)
                : Enum.IsDefined(typeof(MouseAction), parts[^1])
                    ? (object)new MouseWheelGesture(Enum.Parse<MouseAction>(parts[^1]), modifier)
