...
public class GitFileEntryTest
{
    ...
    public void ParseShouldParseGitFileOutput()
    {
        ...
        Assert.That(
            ...
            Is.EquivalentTo(
                new[]
                {
                    PosixFileModes.Read | PosixFileModes.Write,
                    PosixFileModes.Read,
                    PosixFileModes.Read,
                }
            )
        ...
        Assert.That(
            entry.Id,
            Is.EqualTo(GitFileId.FromHash("96d80cd6c4e7158dbebd0849f4fb7ce513e5828c"))
        );
    ...
    public void ParseShouldParseGitDirectoryOutput()
    {
        ...
        Assert.That(
            entry.FileModes,
            Is.EquivalentTo(new[] { PosixFileModes.None, PosixFileModes.None, PosixFileModes.None })
        );
        Assert.That(entry.Type, Is.EqualTo(GitFileType.Tree));
        Assert.That(
            entry.Id,
            Is.EqualTo(GitFileId.FromHash("22af1686db046317f3eea156ca1c547c63febd1b"))
        );
